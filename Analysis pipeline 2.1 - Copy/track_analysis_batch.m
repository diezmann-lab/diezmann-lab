...
% Edited by VK 2026 in the LvD lab 

function track_analysis_batch
s = filesep;
...
cd(expdir)
%% Step 2: regular analysis :)
for f = 1:numFolders
naddir = append(expdir,s,string(expName(f))); % go into each gonad folder
    cd(naddir)
    trackStruct = dir(fullfile(naddir,'*SCOnly*.csv')); % ADJUST MANUALLY
    % [selectedTracksName, selectedTracksPath] = load(trackStruct.name); % load in correct files
    selectedTracksName = readtable(trackStruct.name);
    % tracksOrg = tracksOrg.tracksOrg;

fileSuffixtag = '_SCOnly'; % ADJUST MANUALLY
currentTime = datetime('now','Format','yMMdd-HHmm');
fileSuffix = [fileSuffixtag char(currentTime)];
...
testNames = {'x','y','z','t','id','precisionx','precisiony','precisionz','seg_id','track_id','cycle','inSC'};
    fullPath = append(naddir,s,trackStruct.name);
    ...
    dataAll = readtable(fullPath,opts);
...
options = 'track'; %'segment' --> for SConly;
...
for i = 1:max(splitBy)
    ...
    frame = t(splitBy==i);
    molID = id(splitBy==i);
    cycleID = cycle(splitBy==i);
    inSCID = inSC(splitBy==i);
    ...
    while length(frame)<(frame(end)-frame(1)+1)
        ...
        cycleID = [cycleID(1:idx); nan(frameDiff(idx),1); cycleID(idx+1:end)];
        inSCID = [inSCID(1:idx); nan(frameDiff(idx),1); inSCID(idx+1:end)];
    end
    ...
    jd = sqrt(xDiffs(:,1).^2+yDiffs(:,1).^2); 
    mjd = mean(sqrt(xDiffs(:,1).^2+yDiffs(:,1).^2),'omitnan'); 
    msd = mean(xDiffs.^2+yDiffs.^2,'omitnan')/(1000^2); % convert to um. 
    % note that this is not the same: mjd = sqrt(msd(:,1));

    % generate per-track MSD. note this can fail with short tracks

    if length(msd) < minLocNum
        fitD = nan;
        fitSigma = nan;

    elseif any(isnan(msd(1:lagsToFit))) 
        fitD = nan;
        fitSigma = nan;
    else
    ...
    trackInfo(i).cycle = cycleID;
    trackInfo(i).inSC = inSCID;
...
% trackInfo = [tracksAll{:}];
ind = [trackInfo.length] >= minLocNum;
trackInfo = trackInfo(ind);
...
for i = 1:length(trackInfo)
    ...
    msd1 = mean(p1diffs.^2,'omitnan')/(1000^2); % convert to um
    msd2 = mean(p2diffs.^2,'omitnan')/(1000^2); % convert to um
...
MSDx = mean(allLagsX.^2,1,'omitnan');
MSDy = mean(allLagsY.^2,1,'omitnan');
...
% track lengths
%{
tLengths = [tracksToUse(:).length];
h = figure; hist(tLengths);
saveas(h, ['track_lengths' fileSuffix],'fig'); close(h);
medLength = median(tLengths);
meanLength = mean(tLengths);
stdLength = std(tLengths);
%}
...
save(append(num2str(f),'tracksToUse',fileSuffixtag),'tracksToUse')
end